...
var relatorio_15_juridico = SuperWidget.extend({
    ...
    loadTable: function (htmlElement, event) {
        ...
        let ZATENDENTE = String($(`[name='ZATENDENTE_${this.instanceId}']`).val()).trim();
        ...
        let ZFONTE = String($(`[name='ZFONTE_${this.instanceId}']`).val()).trim();
        ...
        if (ZATENDENTE != "") constraints.push(DatasetFactory.createConstraint('ZATENDENTE', ZATENDENTE, ZATENDENTE, ConstraintType.MUST, true));
        ...
        if (ZFONTE != "") constraints.push(DatasetFactory.createConstraint('ZFONTE', ZFONTE, ZFONTE, ConstraintType.MUST, true));
        ...
        dadosOriginais = DatasetFactory.getDataset("dsWidgetRel15Juridico", null, constraints, null);

        // var dadosOriginais = DatasetFactory.getDataset("dsWidgetRel15Juridico", null, [
        //     DatasetFactory.createConstraint('sqlLimit', 100, 100, ConstraintType.MUST),
        //     // DatasetFactory.createConstraint('NUM_PROCES', 14907, 14907, ConstraintType.MUST),
...
function agruparObjeto(objeto, NUM_PROCES) {
    ...
    objeto.values.forEach(item => {
        ...
        novoObjeto.Avaliado = mudaAvalicao(item.Avaliado);
